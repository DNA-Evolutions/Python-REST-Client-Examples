...
class Constraint(BaseModel):
    ...
    """ # noqa: E501
    initial_class_name: Optional[StrictStr] = Field(default=None, description="A constraint can transform during json serialization and deserialization. For example, the user changes the isHard property. To reflect this, on next loading via JSON the constraint is transformed. The initial class name helps to understand the origin of the constraint. to be fulfilled.", alias="initialClassName")
    type: ConstraintType
    is_hard: StrictBool = Field(description="Defines if a constraint is soft contrained or hard constraint. A hard constraint is fullfilled by the architectural design of the Optimzer. Therefore a hard constraint violation cannot occure. However, not all constraints can be hard-constraint-", alias="isHard")
    __properties: ClassVar[List[str]] = ["initialClassName", "type", "isHard"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if initial_class_name (nullable) is None
        # and model_fields_set contains the field
        if self.initial_class_name is None and "initial_class_name" in self.model_fields_set:
            _dict['initialClassName'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "initialClassName": obj.get("initialClassName"),
            "type": ConstraintType.from_dict(obj["type"]) if obj.get("type") is not None else None,
            "isHard": obj.get("isHard")
        })
        return _obj
