...
class AbsoluteNodeColorMultiRouteConstraint(ConstraintType):
    """
    ...
    absolute_node_color_capacities: List[AbsoluteNodeColorCapacity] = Field(description="The per-color capacity limits (absolute count-based) enforced by this multi-route constraint.", alias="absoluteNodeColorCapacities")
    type_name: StrictStr = Field(description="The typeName of the object", alias="typeName")
    __properties: ClassVar[List[str]] = ["typeName", "mirrorItem", "absoluteNodeColorCapacities"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in absolute_node_color_capacities (list)
        _items = []
        if self.absolute_node_color_capacities:
            for _item_absolute_node_color_capacities in self.absolute_node_color_capacities:
                if _item_absolute_node_color_capacities:
                    _items.append(_item_absolute_node_color_capacities.to_dict())
            _dict['absoluteNodeColorCapacities'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "typeName": obj.get("typeName") if obj.get("typeName") is not None else 'AbsoluteNodeColorMultiRouteConstraint',
            "mirrorItem": IntegerMultiConstraintHelperItem.from_dict(obj["mirrorItem"]) if obj.get("mirrorItem") is not None else None,
            "absoluteNodeColorCapacities": [AbsoluteNodeColorCapacity.from_dict(_item) for _item in obj["absoluteNodeColorCapacities"]] if obj.get("absoluteNodeColorCapacities") is not None else None
        })
        return _obj
