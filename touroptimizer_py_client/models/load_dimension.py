...
class LoadDimension(BaseModel):
    ...
    """ # noqa: E501
    unload_all_dimension: StrictInt = Field(description="The number of load dimensions that participate in the unload-all operation. For example, if the resource carries goods across 3 dimensions (weight, volume, pallets) and unloadAllDimension is set to 2, only the first 2 dimensions are emptied at this node.", alias="unloadAllDimension")
    total_load_dimension: StrictInt = Field(description="The total number of load dimensions defined for this node. Must match the dimensionality of the resource's capacity vector and the node's load vector.", alias="totalLoadDimension")
    unload_all: Optional[StrictBool] = Field(default=None, description="If true, the resource unloads all remaining cargo at this node before continuing. Typically used at depot-return nodes or redistribution hubs in PND scenarios.", alias="unloadAll")
    __properties: ClassVar[List[str]] = ["unloadAllDimension", "totalLoadDimension", "unloadAll"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "unloadAllDimension": obj.get("unloadAllDimension"),
            "totalLoadDimension": obj.get("totalLoadDimension"),
            "unloadAll": obj.get("unloadAll")
        })
        return _obj
