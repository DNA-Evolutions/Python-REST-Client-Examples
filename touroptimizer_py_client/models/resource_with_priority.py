...
class ResourceWithPriority(BaseModel):
    """
    The list of resources that should NOT visit a certain node.
    """ # noqa: E501
    resource_id: StrictStr = Field(description="The resourceId that will be part of a constraint. For example, Jack is a preffered Resource of a node. If multiple resources are preferred, the Optimizer tries to schedule the Resource with the highest priority.", alias="resourceId")
    priority: StrictInt = Field(description="The scheduling priority for this resource within the constraint. When multiple resources are eligible, the optimizer prefers the one with the highest priority value.")
    __properties: ClassVar[List[str]] = ["resourceId", "priority"]
