...
class RangeDayMonth(DateDef):
    """
    Matches any month-day combination within the interval [from, to] (inclusive). typeName must be \"Range.DayMonth\".
    """ # noqa: E501
    var_from: Optional[DayMonth] = Field(default=None, alias="from")
    to: Optional[DayMonth] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "from": DayMonth.from_dict(obj["from"]) if obj.get("from") is not None else None,
            "to": DayMonth.from_dict(obj["to"]) if obj.get("to") is not None else None
        })
        return _obj
