...
class RangeWeekDay(DateDef):
    """
    Matches any ISO weekday within the interval [from, to] (inclusive, wrapping). typeName must be \"Range.WeekDay\".
    """ # noqa: E501
    var_from: Optional[WeekDay] = Field(default=None, alias="from")
    to: Optional[WeekDay] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "from": WeekDay.from_dict(obj["from"]) if obj.get("from") is not None else None,
            "to": WeekDay.from_dict(obj["to"]) if obj.get("to") is not None else None
        })
        return _obj
