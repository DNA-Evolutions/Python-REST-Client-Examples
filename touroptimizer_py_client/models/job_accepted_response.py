...
class JobAcceptedResponse(BaseModel):
    ...
    """ # noqa: E501
    job_id: StrictStr = Field(description="The unique job id token. This should be treated as secret but is not the only security barrier.", alias="jobId")
    creator_hash: Optional[StrictStr] = Field(default=None, description="Thr related creatorHash force-hashed by sha256(). In the optimization result the creator is only hashed, if the magic prefix 'hash:' followed by the creator is used.", alias="creatorHash")
    ...
    submitted_at: Optional[StrictInt] = Field(default=None, description="The time as millis when job was committed.", alias="submittedAt")
    status: Optional[StrictStr] = Field(default=None, description="The status.")
    __properties: ClassVar[List[str]] = ["jobId", "creatorHash", "ident", "submittedAt", "status"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['ACCEPTED', 'REJECTED', 'ERROR']):
            raise ValueError("must be one of enum values ('ACCEPTED', 'REJECTED', 'ERROR')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "jobId": obj.get("jobId"),
            "creatorHash": obj.get("creatorHash"),
            "ident": obj.get("ident"),
