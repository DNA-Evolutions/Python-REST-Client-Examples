...
class GeoPillarNode(PillarType):
    ...
    only_scheduled_in_company: Optional[StrictBool] = Field(default=None, description="If true, this pillar node is only scheduled when the attached resource is part of the same organizational company/group. Prevents cross-company pillar assignments in multi-fleet scenarios.", alias="onlyScheduledInCompany")
    ...
    is_forced_stay_node: Optional[StrictBool] = Field(default=None, description="If true, the optimizer must treat this pillar node as an overnight-stay location. The route will terminate here and the next working-hour route will start from this position.", alias="isForcedStayNode")
    is_auto_transformable2_start_anchor: Optional[StrictBool] = Field(default=None, description="If true, the optimizer may automatically promote this pillar to a route-start anchor when it determines that doing so improves the overall solution quality. Allows flexible anchor assignment without manual configuration.", alias="isAutoTransformable2StartAnchor")
    ...
    is_schedulable_after_working_hours: Optional[StrictBool] = Field(default=None, description="If true, the optimizer is allowed to schedule this pillar node after the resource's official working-hours end. Useful for mandatory end-of-day stops (e.g. vehicle return, paperwork drop-off).", alias="isSchedulableAfterWorkingHours")
    ...
    is_schedulable_before_working_hours: Optional[StrictBool] = Field(default=None, description="If true, the optimizer is allowed to schedule this pillar node before the resource's official working-hours start. Useful for mandatory stops (e.g. depot loading) that must happen pre-shift.", alias="isSchedulableBeforeWorkingHours")
    is_time_adjustable_anchor: Optional[StrictBool] = Field(default=None, description="If true, the anchor timing derived from this pillar is allowed to shift within the working-hour boundaries. Enables the optimizer to adjust the schedule around fixed appointments while maintaining the pillar's role as a route start or termination anchor.", alias="isTimeAdjustableAnchor")
    __properties: ClassVar[List[str]] = ["typeName", "attachedResourceId", "onlyScheduledInCompany", "connectionRelatedLateMargin", "isForcedStayNode", "isAutoTransformable2StartAnchor", "isOverwritingRouteStart", "isSchedulableAfterWorkingHours", "isOverwritingRouteTermination", "isSchedulableBeforeWorkingHours", "isTimeAdjustableAnchor"]
