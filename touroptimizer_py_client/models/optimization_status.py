...
class OptimizationStatus(BaseModel):
    ...
    """ # noqa: E501
    status_description: StrictStr = Field(description="A human-readable description of the optimization outcome (e.g. 'SUCCESS_WITH_SOLUTION', 'Optimization execution failed due to timeout'). Provides more context than the status tag alone.", alias="statusDescription")
    error: StrictStr = Field(description="An error code or message describing the failure cause. Set to 'NO_ERROR' on successful runs. On failure, contains the exception message or a structured error identifier for programmatic handling.")
    status: StrictStr = Field(description="The machine-readable status tag indicating the optimization outcome. UNKNOWN: status not yet determined. ERROR: the run failed. SUCCESS_WITH_SOLUTION: completed with a valid solution. SUCCESS_WITHOUT_SOLUTION: completed but no feasible assignment was found.")
    __properties: ClassVar[List[str]] = ["statusDescription", "error", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['UNKNOWN', 'ERROR', 'SUCCESS_WITH_SOLUTION', 'SUCCESS_WITHOUT_SOLUTION']):
            raise ValueError("must be one of enum values ('UNKNOWN', 'ERROR', 'SUCCESS_WITH_SOLUTION', 'SUCCESS_WITHOUT_SOLUTION')")
        return value
