...
class NodeRelation(BaseModel):
    ...
    """ # noqa: E501
    master_node_id: StrictStr = Field(description="The id of the master node in this relation. The master node serves as the anchor point — constraints are evaluated relative to this node's scheduling (e.g. 'master must be visited before related').", alias="masterNodeId")
    related_node_ids: List[StrictStr] = Field(description="The list of related node ids that are coupled to the master node via this relation. All listed nodes must satisfy the relation type (e.g. all must be on the same route as the master).", alias="relatedNodeIds")
    type: NodeRelationType
    relation_mode: Optional[StrictStr] = Field(default=None, description="The enforcement mode of this relation. STRONG (default) means the optimizer always respects it. WEAK allows the optimizer to violate the relation at a cost if no feasible solution can satisfy it.", alias="relationMode")
