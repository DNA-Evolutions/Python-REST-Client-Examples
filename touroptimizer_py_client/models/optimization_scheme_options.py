...
class OptimizationSchemeOptions(BaseModel):
    """
    ...
    """ # noqa: E501
    construction_optimization_algorithm_config: ConstructionOptimizationAlgorithmConfig = Field(alias="constructionOptimizationAlgorithmConfig")
    heuristic_optimization_algorithm_configs: List[HeuristicOptimizationAlgorithmConfig] = Field(description="An ordered list of heuristic algorithm configurations that are executed sequentially after construction. Each entry defines an algorithm (e.g. Genetic Algorithm, Simulated Annealing) and its parameters (iteration count, repetitions). The optimizer runs each phase in order to progressively improve the solution.", alias="heuristicOptimizationAlgorithmConfigs")
    __properties: ClassVar[List[str]] = ["constructionOptimizationAlgorithmConfig", "heuristicOptimizationAlgorithmConfigs"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of construction_optimization_algorithm_config
        if self.construction_optimization_algorithm_config:
            _dict['constructionOptimizationAlgorithmConfig'] = self.construction_optimization_algorithm_config.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in heuristic_optimization_algorithm_configs (list)
        _items = []
        if self.heuristic_optimization_algorithm_configs:
            for _item_heuristic_optimization_algorithm_configs in self.heuristic_optimization_algorithm_configs:
                if _item_heuristic_optimization_algorithm_configs:
                    _items.append(_item_heuristic_optimization_algorithm_configs.to_dict())
            _dict['heuristicOptimizationAlgorithmConfigs'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "constructionOptimizationAlgorithmConfig": ConstructionOptimizationAlgorithmConfig.from_dict(obj["constructionOptimizationAlgorithmConfig"]) if obj.get("constructionOptimizationAlgorithmConfig") is not None else None,
            "heuristicOptimizationAlgorithmConfigs": [HeuristicOptimizationAlgorithmConfig.from_dict(_item) for _item in obj["heuristicOptimizationAlgorithmConfigs"]] if obj.get("heuristicOptimizationAlgorithmConfigs") is not None else None
        })
        return _obj
