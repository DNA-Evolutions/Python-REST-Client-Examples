...
class MaxDistanceConstructionHook(HookType):
    """
    MaxDistanceConstructionHook
    """ # noqa: E501
    type_name: StrictStr = Field(description="The typeName of the object", alias="typeName")
    prioritize_node_connector_on_fallback_speed: StrictBool = Field(description="In case the route to be used to calculate the average speed is empty, we need to calculate a fallback speed. By default we take the average speed provided by the resource. However, we can also extract a few connections from node connector that belong to the visiting resource of the route and calculated an average speed.", alias="prioritizeNodeConnectorOnFallbackSpeed")
    only_apply_once: StrictBool = Field(description="If only applied once, the hook is autmatically set to be inactive after one execution.", alias="onlyApplyOnce")
    invocation_position: StrictStr = Field(description="When is the hook incorporated?", alias="invocationPosition")
    driven_time: StrictStr = Field(description="The offset for driven time. This time will be transformed into a distance (utilizing average resource speed) that is substracted from the defined maxdistance.", alias="drivenTime")
    driven_distance: StrictStr = Field(description="The offset for driven distance. This value is substracted from the defined maxdistance.", alias="drivenDistance")
    is_active: StrictBool = Field(description="Is the hook active?", alias="isActive")
    __properties: ClassVar[List[str]] = ["typeName", "prioritizeNodeConnectorOnFallbackSpeed", "onlyApplyOnce", "invocationPosition", "drivenTime", "drivenDistance", "isActive"]

    @field_validator('invocation_position')
    def invocation_position_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PRE_CONSTRUCTION', 'BEFORE_LEFTOVER_HANDLING', 'POST_CONSTRUCTION']):
            raise ValueError("must be one of enum values ('PRE_CONSTRUCTION', 'BEFORE_LEFTOVER_HANDLING', 'POST_CONSTRUCTION')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "typeName": obj.get("typeName") if obj.get("typeName") is not None else 'MaxDistanceConstructionHook',
            "prioritizeNodeConnectorOnFallbackSpeed": obj.get("prioritizeNodeConnectorOnFallbackSpeed"),
            "onlyApplyOnce": obj.get("onlyApplyOnce"),
            "invocationPosition": obj.get("invocationPosition"),
            "drivenTime": obj.get("drivenTime"),
            "drivenDistance": obj.get("drivenDistance"),
