...
class ZoneNumberQualification(QualificationType):
    """
    ...
    """ # noqa: E501
    my_code: Optional[ZoneCodeType] = Field(default=None, alias="myCode")
    my_extra_codes: Optional[List[ZoneCodeType]] = Field(default=None, description="Additional zone codes this resource is also qualified to serve, beyond the primary code.", alias="myExtraCodes")
    type_name: StrictStr = Field(description="The typeName of the object", alias="typeName")
    __properties: ClassVar[List[str]] = ["typeName", "myCode", "myExtraCodes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of my_code
        if self.my_code:
            _dict['myCode'] = self.my_code.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in my_extra_codes (list)
        _items = []
        if self.my_extra_codes:
            for _item_my_extra_codes in self.my_extra_codes:
                if _item_my_extra_codes:
                    _items.append(_item_my_extra_codes.to_dict())
            _dict['myExtraCodes'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "typeName": obj.get("typeName") if obj.get("typeName") is not None else 'ZoneNumberQualification',
            "myCode": ZoneCodeType.from_dict(obj["myCode"]) if obj.get("myCode") is not None else None,
            "myExtraCodes": [ZoneCodeType.from_dict(_item) for _item in obj["myExtraCodes"]] if obj.get("myExtraCodes") is not None else None
        })
        return _obj
