...
class ZoneNumberConstraint(ConstraintType):
    """
    ...
    """ # noqa: E501
    zone_codes: List[ZoneCodeType] = Field(description="The list of numeric zone codes that a resource must be qualified for to visit this node. Used in conjunction with ZoneNumberQualification on the resource side.", alias="zoneCodes")
    type_name: StrictStr = Field(description="The typeName of the object", alias="typeName")
    __properties: ClassVar[List[str]] = ["typeName", "zoneCodes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in zone_codes (list)
        _items = []
        if self.zone_codes:
            for _item_zone_codes in self.zone_codes:
                if _item_zone_codes:
                    _items.append(_item_zone_codes.to_dict())
            _dict['zoneCodes'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "typeName": obj.get("typeName") if obj.get("typeName") is not None else 'ZoneNumberConstraint',
            "zoneCodes": [ZoneCodeType.from_dict(_item) for _item in obj["zoneCodes"]] if obj.get("zoneCodes") is not None else None
        })
        return _obj
