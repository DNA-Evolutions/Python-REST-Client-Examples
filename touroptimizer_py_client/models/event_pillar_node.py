...
class EventPillarNode(PillarType):
    """
    ...
    """ # noqa: E501
    attached_resource_id: Optional[StrictStr] = Field(default=None, description="The attached resourceId. An event pillar must be visited by this specific resource. The optimizer guarantees architectural compliance — this is a hard constraint.", alias="attachedResourceId")
    only_scheduled_in_company: Optional[StrictBool] = Field(default=None, description="If true, this event pillar is only scheduled when the attached resource belongs to the same organizational group. Prevents cross-company assignments in multi-fleet scenarios.", alias="onlyScheduledInCompany")
    type_name: StrictStr = Field(description="The typeName of the object", alias="typeName")
    is_extendable_end: Optional[StrictBool] = Field(default=None, description="If true, the event pillar's end time can be extended beyond its defined opening-hour boundary to accommodate scheduling needs. Useful for flexible-duration events like meetings or calls.", alias="isExtendableEnd")
    is_schedulable_after_working_hours: Optional[StrictBool] = Field(default=None, description="If true, the optimizer may schedule this event pillar after the resource's official working-hours end. Enables post-shift mandatory events (e.g. end-of-day reporting, handover calls).", alias="isSchedulableAfterWorkingHours")
    ...
    is_schedulable_before_working_hours: Optional[StrictBool] = Field(default=None, description="If true, the optimizer may schedule this event pillar before the resource's official working-hours start. Enables pre-shift mandatory events (e.g. morning briefings, pre-route phone calls).", alias="isSchedulableBeforeWorkingHours")
    is_time_adjustable_anchor: Optional[StrictBool] = Field(default=None, description="If true, the anchor timing derived from this event pillar is allowed to shift within the working-hour boundaries. Enables the optimizer to adjust the schedule around fixed events while maintaining the pillar's role as a route anchor point.", alias="isTimeAdjustableAnchor")
    __properties: ClassVar[List[str]] = ["typeName", "attachedResourceId", "onlyScheduledInCompany", "isExtendableEnd", "isSchedulableAfterWorkingHours", "isOverwritingRouteTermination", "isSchedulableBeforeWorkingHours", "isTimeAdjustableAnchor"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "typeName": obj.get("typeName") if obj.get("typeName") is not None else 'EventPillarNode',
            "attachedResourceId": obj.get("attachedResourceId"),
            "onlyScheduledInCompany": obj.get("onlyScheduledInCompany"),
            "isExtendableEnd": obj.get("isExtendableEnd"),
