...
class TypeWithExpertiseConstraint(ConstraintType):
    """
    ...
    """ # noqa: E501
    types_with_expertise: List[TypeWithExpertise] = Field(description="The types with expertise", alias="typesWithExpertise")
    cost_model: Optional[StrictStr] = Field(default=None, description="The cost model for matching the expertise.", alias="costModel")
    type_name: StrictStr = Field(description="The typeName of the object", alias="typeName")
    __properties: ClassVar[List[str]] = ["typeName", "typesWithExpertise", "costModel"]

    @field_validator('cost_model')
    def cost_model_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['PENALIZE_MATCHING_SKILL_HIGH_DELTA', 'PENALIZE_MATCHING_SKILL_LOW_DELTA', 'NO_PENALIZE_MATCHING_SKILL']):
            raise ValueError("must be one of enum values ('PENALIZE_MATCHING_SKILL_HIGH_DELTA', 'PENALIZE_MATCHING_SKILL_LOW_DELTA', 'NO_PENALIZE_MATCHING_SKILL')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in types_with_expertise (list)
        _items = []
        if self.types_with_expertise:
            for _item_types_with_expertise in self.types_with_expertise:
                if _item_types_with_expertise:
                    _items.append(_item_types_with_expertise.to_dict())
            _dict['typesWithExpertise'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "typeName": obj.get("typeName") if obj.get("typeName") is not None else 'TypeWithExpertise',
            "typesWithExpertise": [TypeWithExpertise.from_dict(_item) for _item in obj["typesWithExpertise"]] if obj.get("typesWithExpertise") is not None else None,
            "costModel": obj.get("costModel")
        })
        return _obj
