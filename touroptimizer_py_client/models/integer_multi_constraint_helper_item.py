...
class IntegerMultiConstraintHelperItem(BaseModel):
    """
    The mirroring configuration that maps this constraint across working-hour boundaries.
    """ # noqa: E501
    pairs: List[StringIntegerPair] = Field(description="A list of key-value pairs mapping element identifiers to integer values. Used internally to mirror multi-route constraint configurations across working hours.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "pairs": [StringIntegerPair.from_dict(_item) for _item in obj["pairs"]] if obj.get("pairs") is not None else None
        })
        return _obj
