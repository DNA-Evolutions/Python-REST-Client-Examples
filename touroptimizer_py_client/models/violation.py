...
class Violation(BaseModel):
    ...
    """ # noqa: E501
    value: StrictStr = Field(description="If for example a constraint is triggering the violation, the underlying ident of the constraint class might be provided.")
    desc: StrictStr = Field(description="The description of the violation. A human readable description of the violation")
    offender: Optional[StrictStr] = Field(default=None, description="The offender corresponding to the violation. That could be a node or route id.")
    category: StrictStr = Field(description="The category of the violation. The main category of the violation.")
    attribute: StrictStr = Field(description="The attribute is further defining the type of the violation. For example, late and early violation belong to the attribute 'TIMECONSTRAINT'.")
    sub_attribute: StrictStr = Field(description="The subAttribute defines what kind of violation we are dealing with.", alias="subAttribute")
    code: StrictInt = Field(description="The code is the unique code for each Violation type.")
    __properties: ClassVar[List[str]] = ["value", "desc", "offender", "category", "attribute", "subAttribute", "code"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "subAttribute": obj.get("subAttribute"),
            "code": obj.get("code")
        })
        return _obj
