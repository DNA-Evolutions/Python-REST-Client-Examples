...
class RangeDayMonthYear(DateDef):
    """
    Matches any full date within the interval [from, to] (inclusive). typeName must be \"Range.DayMonthYear\".
    """ # noqa: E501
    var_from: Optional[DayMonthYear] = Field(default=None, alias="from")
    to: Optional[DayMonthYear] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "from": DayMonthYear.from_dict(obj["from"]) if obj.get("from") is not None else None,
            "to": DayMonthYear.from_dict(obj["to"]) if obj.get("to") is not None else None
        })
        return _obj
