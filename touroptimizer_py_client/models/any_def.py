...
class AnyDef(DateDef):
    ...
    """ # noqa: E501
    time_defs: Optional[List[AnyDefAllOfTimeDefs]] = Field(default=None, description="List of date predicates evaluated with OR logic. The containing AnyDef matches if at least one entry matches.", alias="timeDefs")
    __properties: ClassVar[List[str]] = ["typeName", "timeDefs"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in time_defs (list)
        _items = []
        if self.time_defs:
            for _item_time_defs in self.time_defs:
                if _item_time_defs:
                    _items.append(_item_time_defs.to_dict())
            _dict['timeDefs'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "timeDefs": [AnyDefAllOfTimeDefs.from_dict(_item) for _item in obj["timeDefs"]] if obj.get("timeDefs") is not None else None
        })
        return _obj

from touroptimizer_py_client.models.any_def_all_of_time_defs import AnyDefAllOfTimeDefs
# TODO: Rewrite to not use raise_errors
AnyDef.model_rebuild(raise_errors=False)
