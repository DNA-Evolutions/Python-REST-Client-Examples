...
class ConnectionRelatedLateMargin(BaseModel):
    """
    ...
    """ # noqa: E501
    shift_element_connection_realted_late_margin: Optional[StrictBool] = Field(default=None, description="If true, the optimizer shifts the element's effective time window by the computed late margin, rather than simply absorbing the lateness as a soft violation. Produces a cleaner schedule representation.", alias="shiftElementConnectionRealtedLateMargin")
    apply_partial_connection_related_late_margin: Optional[StrictBool] = Field(default=None, description="If true, partial late margins are applied proportionally based on the actual connection time relative to the maximum margin. If false, the full margin is applied as a binary threshold.", alias="applyPartialConnectionRelatedLateMargin")
    max_margin: StrictStr = Field(description="The maximum late-arrival margin that can be granted, regardless of how long the inbound connection is. Caps the proportional margin to prevent excessively permissive lateness tolerance.", alias="maxMargin")
    margin_factor: Union[StrictFloat, StrictInt] = Field(description="A factor between 0.0 and 1.0 that determines what proportion of the inbound connection time is granted as late-arrival margin. For example, a factor of 0.2 on a 50-minute connection yields a 10-minute margin (capped by maxMargin).", alias="marginFactor")
    __properties: ClassVar[List[str]] = ["shiftElementConnectionRealtedLateMargin", "applyPartialConnectionRelatedLateMargin", "maxMargin", "marginFactor"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "shiftElementConnectionRealtedLateMargin": obj.get("shiftElementConnectionRealtedLateMargin"),
            "applyPartialConnectionRelatedLateMargin": obj.get("applyPartialConnectionRelatedLateMargin"),
            "maxMargin": obj.get("maxMargin"),
