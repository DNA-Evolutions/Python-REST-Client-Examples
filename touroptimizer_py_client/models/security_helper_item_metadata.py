...
class SecurityHelperItemMetadata(BaseModel):
    ...
    """ # noqa: E501
    enc_mode: Optional[StrictStr] = Field(default=None, description="The encryption mode that was used. 'CLIENT' = key derived from a client passphrase via PBKDF2. 'KMS' = key is a server-generated DEK wrapped by an external KMS. Old metadata without this field defaults to CLIENT for backward compatibility.", alias="encMode")
    iv: StrictStr = Field(description="The IV used for AES-GCM initialization as base64-encoded String. Should be 12 bytes (96 bits) for GCM per NIST SP 800-38D.")
    enc_algo: StrictStr = Field(description="The algorithm used for encryption.", alias="encAlgo")
    key_length: Optional[StrictInt] = Field(default=None, description="AES key length in bits.", alias="keyLength")
    salt: Optional[StrictStr] = Field(default=None, description="The salt used for PBKDF2 key derivation as base64-encoded String. Only present in CLIENT mode. Empty in KMS mode.")
    secret_key_fac_algo: Optional[StrictStr] = Field(default=None, description="Secret Key Factory algorithm. Only used in CLIENT mode.", alias="secretKeyFacAlgo")
    secret_key_spec_algo: Optional[StrictStr] = Field(default=None, description="Secret Key Spec algorithm.", alias="secretKeySpecAlgo")
    iteration_count: Optional[StrictInt] = Field(default=None, description="PBKDF2 iteration count. Only used in CLIENT mode. Old snapshots without this field used 65536. KMS mode sets this to 0.", alias="iterationCount")
    wrapped_dek: Optional[StrictStr] = Field(default=None, description="The data encryption key (DEK) wrapped (encrypted) by the tenant's key encryption key (KEK) via an external KMS, stored as a base64-encoded String. Only present in KMS mode. The plaintext DEK is never stored it must be unwrapped via the KMS before decryption.", alias="wrappedDek")
    kek_id: Optional[StrictStr] = Field(default=None, description="The identifier (URI or ARN) of the key encryption key (KEK) in the external KMS that was used to wrap the DEK. Only present in KMS mode. Required for unwrapping the DEK at retrieval time. Example for Azure Key Vault: 'https://myvault.vault.azure.net/keys/tenant-kek/abc123'. Example for AWS KMS: 'arn:aws:kms:eu-west-1:123456789:key/mrk-abc123'.", alias="kekId")
    kms_mode: Optional[StrictBool] = Field(default=None, description="Convenience flag derived from encMode. True when encMode is KMS, false for CLIENT mode or legacy documents without an encMode field.", alias="kmsMode")
    __properties: ClassVar[List[str]] = ["encMode", "iv", "encAlgo", "keyLength", "salt", "secretKeyFacAlgo", "secretKeySpecAlgo", "iterationCount", "wrappedDek", "kekId", "kmsMode"]
