...
class NodeType(BaseModel):
    ...
    __discriminator_value_class_map: ClassVar[Dict[str, str]] = {
        'Event': 'EventNode','Geo': 'GeoNode','IdleEvent': 'IdleEventNode'
    }
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Optional[Union[EventNode, GeoNode, IdleEventNode]]:
    ...
    @classmethod
    def from_dict(cls, obj: Dict[str, Any]) -> Optional[Union[EventNode, GeoNode, IdleEventNode]]:
        """Create an instance of NodeType from a dict"""
        # look up the object type based on discriminator mapping
        object_type = cls.get_discriminator_value(obj)
        if object_type ==  'EventNode':
            return import_module("touroptimizer_py_client.models.event_node").EventNode.from_dict(obj)
        if object_type ==  'GeoNode':
            return import_module("touroptimizer_py_client.models.geo_node").GeoNode.from_dict(obj)
        if object_type ==  'IdleEventNode':
            return import_module("touroptimizer_py_client.models.idle_event_node").IdleEventNode.from_dict(obj)

        raise ValueError("NodeType failed to lookup discriminator value from " +
